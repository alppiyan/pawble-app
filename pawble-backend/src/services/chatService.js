...
export const chatService = {
  async listConversations(userId) {
    const [messagePartners, matchPartners] = await Promise.all([
      messageRepository.listConversationPartners(userId),
      likeRepository.listMatchPartners(userId),
    ]);

    const byId = new Map();
    for (const p of messagePartners) {
      byId.set(p.otherUserId, { otherUserId: p.otherUserId, lastMessageTime: p.lastMessageTime });
    }
    for (const p of matchPartners) {
      if (!byId.has(p.otherUserId)) {
        byId.set(p.otherUserId, { otherUserId: p.otherUserId, lastMessageTime: p.matchedAt });
      }
    }
    const partners = [...byId.values()].sort(
    ...
    const enriched = await Promise.all(
      partners.map(async (p) => {
        ...
        return {
          ...
          time: lastMsg ? formatTime(p.lastMessageTime) : '',
        };
      }),
    );
